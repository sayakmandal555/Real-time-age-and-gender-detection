...
image_files = [f for f in glob.glob(r'F:\Minor_project\Age_dataset' + "/**/*", recursive=True) if not os.path.isdir(f)]
...
for img in image_files:
    ...
    label = img.split(os.path.sep)[-4] #F:\Gender\gender_dataset_face\woman\face_1162.jpg
    if label == "51-100":
        label = 3
    elif label == "31-50":
        label = 2
...
def build(width, height, depth, classes):
    ...
    model.add(Flatten()) #flatten is used for dence layer. convert 2d to 1d
...
model.save('Age_classification.model')
